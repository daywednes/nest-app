...
export class TagsRepository extends Repository<TagsEntity> {
  ...
  async getTags(
    ...
    try {
      ...
      this.logger.error(
        `Failed to get Tags for user ${user.username} with dto ${JSON.stringify(
          tagsFilterDto,
        )}`,
        err.stack,
      );
  ...
  async getTagsByDeviceId(deviceId: number, user: User): Promise<TagsEntity[]> {
    const query = this.createQueryBuilder('tags_entity').leftJoinAndSelect(
      'tags_entity.tagsdevice',
      'tagsdevice',
    );
    ...
    try {
      ...
      this.logger.error(`Failed to get Tags for device ${deviceId}`, err.stack);
      throw new InternalServerErrorException();
    }
  }

  async createTags(
    createTagsDto: CreatTagsDto,
    user: User,
  ): Promise<TagsEntity> {
